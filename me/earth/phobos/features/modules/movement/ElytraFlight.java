...
public class ElytraFlight extends Module {
  ...
  public void onUpdate() {
    if (this.mode.getValue() == Mode.BYPASS && ((Integer)this.devMode.getValue()).intValue() == 1 && 
    ...
    switch (event.getStage()) {
      case 0:
        ...
        if (((Boolean)this.autoStart.getValue()).booleanValue() && 
          mc.gameSettings.keyBindJump.isKeyDown() && !mc.player.isElytraFlying() && 
          mc.player.motionY < 0.0D && 
          this.timer.passedMs(250L)) {
          mc.getConnection().sendPacket((Packet)new CPacketEntityAction((Entity)mc.player, CPacketEntityAction.Action.START_FALL_FLYING));
          this.timer.reset();
        } 
        if (this.mode.getValue() == Mode.BETTER) {
          ...
          switch (((Integer)this.devMode.getValue()).intValue()) {
            ...
            case 3:
              if (mc.player.isElytraFlying()) {
                if (this.flyHeight == null || this.posX == null || this.posX
                  .doubleValue() == 0.0D || this.posZ == null || this.posZ
                  .doubleValue() == 0.0D) {
                  this.flyHeight = Double.valueOf(mc.player.posY);
  ...
  public enum Mode {
    VANILLA, PACKET, BOOST, FLY, BYPASS, BETTER, OHARE, TOOBEE;
  }
}
